...
| 8. **Full suite** + **live harness smoke** per Validation strategy #6 and #7. — **✅ DONE 2026-08-23 (with one caveat): full suite green in a complete fastmcp-4 env (529 passed; required fixing a raw-`Path` sys.path entry in ragmcp_fastmcp.py that crashed hypothesis imports, and converting two legacy `get_event_loop()` test helpers to local loops) and on the 3.4.2 interpreter (525 passed / 4 skipped, era module skips). Live smoke: launcher started under the b3 venv, all six tools up on 8000–8005; auth matrix on simplemcp:8002 = no-auth 401 / Bearer 200 / X-API-Key-only 200 (Q9 fix verified live) / wrong-key 401; legacy-handshake client negotiated 2025-11-25 (re-confirmed by probe_eras.py on 2026-08-24; the earlier "2025-03-26" was a mis-transcription — 2025-03-26 is a real spec revision, just not what this stack negotiates: the installed SDK's handshake set is 2024-11-05 / 2025-03-26 / 2025-06-18 / 2025-11-25). Caveat resolved 2026-08-27: the pending native ZCode-client call went through — square/double/greet all returned correct results over the running b3 stack (no restart needed), so the harness-client path is verified end-to-end.** | All green. | — |
| 9. **Roll out** per the Rollout section (merge to `main` once stable FastMCP 4 ships). — **✅ DONE 2026-08-24, EARLY ROLLOUT (user-approved): `feature/fastmcp-4` fast-forwarded onto `main` and pushed while stable had not shipped (still 3.4.7); runtime env already on 4.0.0b3. Old main preserved at branch + annotated tag `fastmcp-3-final` (both = commit `c02d4df`; if ever lost: `git branch <name> c02d4df`). Follow-ups when 4.0.0 stable ships: ✅ ALL DONE 2026-09-02 (stable shipped on PyPI 2026-08-31) — pins bumped b3→4.0.0 (root + tools/memorymcp), Q6 re-verified on stable (live probe of `_server_instances`/`terminate()` + flush endpoint, 8/8 flush tests), full suite green in a fresh venv (580 passed; the 5 failures are the environmental live-server tests, parity-proven identical under b3), lifespan fallback already deleted (2daa20b), beta-pin comments cleaned. Remaining operator steps: restart the launcher on stable (runtime env still has b3) + live auth matrix; note `.kilo/worktrees` pollutes `requirements_manager.py merge` (SKIP_DIRS lacks `.kilo`).** | Released. | — |

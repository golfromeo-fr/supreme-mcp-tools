#!/usr/bin/env python3
"""
Enhanced Web MCP Server - Streamable HTTP Transport
...
# ============================================================================
# Web MCP Streamable HTTP Transport Implementation
# ============================================================================

class WebMCPStreamableHttp(StreamableHttpTransportBase):
    """
    Web MCP server implementation using Streamable HTTP transport.
    
    This class provides enhanced web search and URL fetch tools using
    the Streamable HTTP transport with JSON-RPC framing.
    """
    
    def __init__(self):
        """Initialize the Web MCP Streamable HTTP server."""
                    ...
                    yield {
                        ...
                        "result": {
                            "content": [
                                {
                                    "type": "text",
                                    "text": json.dumps(llm_results, indent=2)
                                }
                            ]
                        }
                    }
        ...
        if timeout < 1 or timeout > 300:
            yield {
                ...
                "error": {
                    "code": -32602,
                    "message": "Invalid params",
                    "data": "timeout must be between 1 and 300 seconds"
                }
            }
            return
        if max_length < 100 or max_length > 1000000:
            yield {
                ...
                "error": {
                    "code": -32602,
                    "message": "Invalid params",
                    "data": "max_length must be between 100 and 1000000 characters"
                }
            }
            return
        if max_size < 1024 or max_size > 50 * 1024 * 1024:
            yield {
                ...
                "error": {
                    "code": -32602,
                    "message": "Invalid params",
                    "data": "max_size must be between 1KB and 50MB"
                }
            }
            return
                ...
                if content_size > max_size:
                    ...
                    yield {
                        ...
                        "error": {
                            "code": -32602,
                            "message": "Invalid params",
                            "data": f"Content size ({content_size:,} bytes) exceeds maximum allowed size ({max_size:,} bytes)."
                        }
                    }
                    return
...
async def lifespan(app: FastAPI):
    ...
    if FEF_V3_AVAILABLE and fef_http_server:
        try:
            ...
            logger.info("FEF V3 management server started on port 9001")
        except Exception as e:
    ...
    # Shutdown
    logger.info("Web MCP Streamable HTTP server shutting down...")
...
app = FastAPI(
    title="Web MCP Streamable HTTP Server",
    description="Enhanced MCP tools for web search and URL fetching using Streamable HTTP transport",
    version="1.0.0",
    lifespan=lifespan
)
...
def get_search_stats(params: dict[str, Any]) -> dict[str, Any]:
    ...
    avg_search_time = (
        ...
        if webmcp_metrics["search_count"] > 0 else 0.0
    )
    return {
...
def get_fetch_stats(params: dict[str, Any]) -> dict[str, Any]:
    ...
    avg_fetch_time = (
        ...
        if webmcp_metrics["fetch_count"] > 0 else 0.0
    )
    return {
        "total_fetches": webmcp_metrics["fetch_count"],
        "fetch_errors": webmcp_metrics["fetch_errors"],
...
def get_search_history(params: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "recent_searches": webmcp_metrics.get("recent_searches", [])[-limit:],
        "total": len(webmcp_metrics.get("recent_searches", []))
    }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Web MCP Streamable HTTP Server")
    parser.add_argument(
        "--host",
        default="0.0.0.0",
